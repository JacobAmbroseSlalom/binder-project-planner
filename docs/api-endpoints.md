...
| Method | Path | Description |
| --- | --- | --- |
| `GET` | `/card-catalog/search?query={query}` | Searches TCGdex through the backend and returns normalized provider cards in provider order. The trimmed query is required and must meet the configured minimum length. |
| `POST` | `/binders/{binderId}/cards` | Creates one binder-owned card. Supports JSON TCGdex-card creation with a normalized catalog result and placement, or multipart custom-card creation with metadata, optional placement, and a required image. Returns `201 Created`, a `Location` header, and the persisted card. |
| `POST` | `/binders/{binderId}/cards/bulk` | Creates independent unplaced TCGdex cards from the complete search-result array and optional shared variation. Returns per-card outcomes with `201 Created` for complete success or `207 Multi-Status` for card-level failures; requires a client idempotency key. |
...
| Method | Path | Description |
| --- | --- | --- |
| `POST` | `/binders/{binderId}/exports/pdf` | Generates the binder-layout PDF from a request-start persisted snapshot. Accepts JSON `{ "includeVariations": boolean }`, with `false` as the default, and returns a downloadable PDF named from the sanitized binder name. |
| `POST` | `/binders/{binderId}/exports/art-pdf` | Generates a downloadable placed-art-only PDF from a request-start persisted snapshot. The filename is the sanitized binder name followed by `-art.pdf`; the request fails when no placed art exists. |
