...
// slot counts already embedded in the binder summary (no separate request)
// and derives the slot-completion percentage on the client, rounded to the
// nearest whole percent. The card-acquisition percentage is deferred to
// story 36.
export function BinderCompletionMetrics({ binder }: { binder: BinderSummary }) {
  const { totalSlots, occupiedSlots } = binder;
