...
// Parses a `width`/`height`/`pages` query-parameter string into a positive
// integer, or returns undefined when it's absent or invalid - letting the
// caller fall back to the form's normal default for that field instead.
// Used by the Finances Preview page's (story 54) "Create binder" button,
// which prefills these 3 fields from its own selected/created Binder cost
// entry's dimensions; every other field (including name) keeps its normal
// empty/default value.
...
export function NewBinderForm() {
  const router = useRouter();
  const { start } = useSaveStatusToast();
  const searchParams = useSearchParams();

  // Query-param overrides layered on top of the normal defaults. Computed
  // directly at render time (rather than synced via an effect) since
  // `useForm`'s `defaultValues` are only ever read once, at this
  // component's initial render.
  const initialValues: BinderDetailsFormInput = {
    ...defaultBinderDetailsFormValues,
    width:
      parsePositiveIntegerParam(searchParams.get('width')) ?? defaultBinderDetailsFormValues.width,
    height:
      parsePositiveIntegerParam(searchParams.get('height')) ??
      defaultBinderDetailsFormValues.height,
    pages:
      parsePositiveIntegerParam(searchParams.get('pages')) ?? defaultBinderDetailsFormValues.pages,
  };
